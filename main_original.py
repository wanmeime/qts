...
@app.get("/v1/market/overview/report")
def get_market_overview_report():
    """全市场扫描 - 返回 Markdown 格式的报告"""
    from tradingagents.dataflows.providers.market_scanner import MarketScanner
    from fastapi.responses import PlainTextResponse
    scanner = MarketScanner()
    result = scanner.scan_all()
    report = scanner.to_markdown(result)
    return PlainTextResponse(report)



@app.get("/v1/market/insight")
async def get_market_insight():
    """全市场洞察 - 多 Agent 协作分析报告"""
    from tradingagents.dataflows.providers.market_scanner import MarketScanner
    from tradingagents.graph.market_multi_agent import analyze_market_with_agents
    from fastapi.responses import PlainTextResponse
    scanner = MarketScanner()
    try:
        market_data = scanner.scan_all()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"市场数据获取失败: {str(e)}")
    try:
        report = await analyze_market_with_agents(market_data)
        return PlainTextResponse(report)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"洞察报告生成失败: {str(e)}")
